...
def generate_section_block(menu_pdf, df, section_title, dict_dim, end_line=True)-> int:
    ...
    for index, row in df.iterrows():
        ...
        dish_desc = ", " + str(row["LB_DISH_SHORT_DESC"])
...
def generate_menu_lunch_pdf(df_suggestion:pd.DataFrame, df_entree:pd.DataFrame, df_plat:pd.DataFrame, df_dessert:pd.DataFrame) -> None:
    ...
    DIM = generate_section_block(menu_pdf, df_suggestion, section_title="Suggestions (hors formule)", dict_dim=DIM, end_line=False)

    menu_pdf.setFont("Helvetica-Bold", 16)
    menu_pdf.drawCentredString(width/2
                                , DIM['y_current']
                                , 'Formule du midi 18€50') 
    DIM["y_current"] += DIM["y_inter_section"]*0.5
    menu_pdf.setFont("Helvetica-Bold", 8)
    menu_pdf.drawCentredString(width/2
                                , DIM['y_current']
                                , '( entrée + plat ou plat + dessert)') 
    DIM["y_current"] += DIM["y_inter_section"]*0.35
    menu_pdf.setFont("Helvetica-Bold", 6)
    menu_pdf.drawCentredString(width/2
                                , DIM['y_current']
                                , 'du lundi au vendredi sauf jours fériés') 
    DIM["y_current"] += DIM["y_inter_section"]*1.0
